function R = InfectiousnessfromInfectionTestingFreqRTPCRFollowup(ttemp,utemp,timet,testtype,R0S,R0A,pA,ts,tL,td,SelfIsolate,betaPCR)
...
for jj=1:length(timet)
    SA(jj,:) = TestSensitivity(u+timet(jj),ts,tL,Senstd,[testtype{jj}],betaPCR);
    SARTPCR(jj,:) = TestSensitivity(u+timet(jj),ts,tL,Senstd,[],betaPCR);
end
...
for jj=1:length(timet)
    SS(jj,:) = TestSensitivity(u+timet(jj),ts,tL,Senstd,[testtype{jj}],betaPCR);
    SSRTPCR(jj,:) = TestSensitivity(u+timet(jj),ts,tL,Senstd,[],betaPCR);
end
