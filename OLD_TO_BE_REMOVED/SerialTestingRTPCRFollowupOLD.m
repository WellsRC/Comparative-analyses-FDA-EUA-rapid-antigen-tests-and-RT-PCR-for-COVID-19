function [RS,RA,RSRF,RARF] = SerialTestingRTPCRFollowupOLD(testtype,timetoff,R0S,R0A,ts,tL,td,SelfIsolate,NT,dT,delayRTPCR)
...
parfor ii=0:(NT-2)
    ...
    RSRF(ii+2)=integral2(@(u,tx)InfectiousnessfromInfectionTestingFreqRTPCRFollowupOLD(tx,u,dayT,testtype,R0S,R0A,0,ts,tL,td,SelfIsolate),0,dT,@(u)min((u+dT.*(ii)+delayRTPCR),(u+dT.*(ii+1))),@(u)(u+dT.*(ii+1)))./dT;
    RARF(ii+2)=integral2(@(u,tx)InfectiousnessfromInfectionTestingFreqRTPCRFollowupOLD(tx,u,dayT,testtype,R0S,R0A,1,ts,tL,td,0),0,dT,@(u)min((u+dT.*(ii)+delayRTPCR),(u+dT.*(ii+1))),@(u)(u+dT.*(ii+1)))./dT;
end
...
RSRF(end)=integral2(@(u,tx)InfectiousnessfromInfectionTestingFreqRTPCRFollowupOLD(tx,u,dayT,testtype,R0S,R0A,0,ts,tL,td,SelfIsolate),0,dT,@(u)min((u+dT.*(NT-1))+delayRTPCR,td),td)./dT;
RARF(end)=integral2(@(u,tx)InfectiousnessfromInfectionTestingFreqRTPCRFollowupOLD(tx,u,dayT,testtype,R0S,R0A,1,ts,tL,td,0),0,dT,@(u)min((u+dT.*(NT-1))+delayRTPCR,td),td)./dT;
end
