...
load([num2str(1) '-day_Delay_Testing_Frequency_RTPCR_NatComm.mat']);
R(:,1)=(1-pA).*RTotS+pA.*RTotA;

RNoTest=integral(@(t)InfectiousnessfromInfection(t,R0S,R0A,pA,ts,tL,td,1),0,inf);
RRTPCRDelay=R(:,1);
load('Testing_Frequency_LumiraDX (Anterior Nasal Swab)_NatComm.mat')
R(:,2)=(1-pA).*RTotS+pA.*RTotA;

load('Testing_Frequency_Sofia (FDA)_NatComm.mat')
R(:,3)=(1-pA).*RTotS+pA.*RTotA;

load('Testing_Frequency_BinaxNOW (FDA)_NatComm.mat')
R(:,4)=(1-pA).*RTotS+pA.*RTotA;

load('Testing_Frequency_BD Veritor_NatComm.mat')
R(:,5)=(1-pA).*RTotS+pA.*RTotA;

load('Testing_Frequency_CareStart (Anterior Nasal Swab - FDA)_NatComm.mat')
...
for zz=0:5
    load([num2str(zz) '-day_Delay_Testing_Frequency_RTPCR_NatComm.mat']);
...
for kk=1:NumTest
    load(['Testing_Frequency_' testName{kk} '_NatComm.mat'],'RTotS','RTotA');
