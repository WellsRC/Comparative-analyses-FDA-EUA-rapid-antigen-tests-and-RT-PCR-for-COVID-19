function S = TestSensitivity(t,ts,AgCutoff,testtype,beta)
...
mm=(Gx(1)-1).*Gx(2);
 ...
 [S]=PCRSens(t,beta);
 V=ViralShedding_Symptomatic(t,inf); % set to inf as we need to conduct the mapping
if(~isempty(testtype))
    tt=[mm:0.1:90]; % made coarsered to improve the search tt=[ts:0.001:90]; 
    Vx=ViralShedding_Symptomatic(tt,inf); % set to inf as we need to conduct the mapping
