function R = InfectiousnessfromInfectionTesting_EstimateOffshore(ttemp,utemp,timet,testtype,RTPCRFollow,R0S,R0A,pA,ts,td,SelfIsolate,RTPCRmodel)
...
for jj=1:length(timet)
    SA(jj,:) = TestSensitivity_RTPCRConfrim(u+timet(jj),ts,[testtype{jj}],RTPCRFollow(jj),RTPCRmodel);
end
...
for jj=1:length(timet)
    SS(jj,:) = TestSensitivity_RTPCRConfrim(u+timet(jj),ts,[testtype{jj}],RTPCRFollow(jj),RTPCRmodel);
end
