...
for ii=1:NumTest
    ...
    load(['Testing_Frequency_' testName{ii} '_DeltaVOC_Uncertainty.mat'],'RTotSv','RTotAv')
...
for fr=4:-1:1
    ...
    if(fr<=2)
        set(gca,'LineWidth',2,'Tickdir','out','Fontsize',20,'XTick',[1:(NumTest+1)],'Xminortick','off','YTick',[0:0.2:1.8],'ylim',[0 1.8],'Yminortick','on','XTickLabel',testname2);
        xtickangle(90)
        xlabel('Test name','Fontsize',26,'Position',[9 -1.45 0]);
    else
        set(gca,'LineWidth',2,'Tickdir','out','Fontsize',20,'XTick',[1:(NumTest+1)],'Xminortick','off','YTick',[0:0.2:1.8],'ylim',[0 1.8],'Yminortick','on','XTickLabel','');
    end
    ylabel({'Effective','reproduction number'},'Fontsize',26,'Position',[-0.998673431830746,0.9,-1]);
    if(fr==1)
