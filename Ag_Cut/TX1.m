...
for TestN=1:NumTests
    ...
    parfor jj=1:14 
        IDSLS(jj)=((1./ts).*integral2(@(u,t)InfectiousnessfromInfectionTesting(t+u,u,[q(jj)],testtype,AgCutoffPSO,R0S,R0A,0,ts,td,SelfIsolate,betaRTPCR),0,ts,q(jj),inf));
        IDSLA(jj)=((1./td).*integral2(@(u,t)InfectiousnessfromInfectionTesting(t+u,u,[q(jj)],testtype,AgCutoffPSO,R0S,R0A,1,ts,td,0,betaRTPCR),0,td,q(jj),inf));  
    end

    save(['TestingonExit_' testName{TestN} '_NoDelay_AgCutoff=' num2str(AgCutoffPSO) '.mat']);
end
