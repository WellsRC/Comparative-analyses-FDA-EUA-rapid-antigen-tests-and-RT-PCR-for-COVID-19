function SIFigureTest_Uncertainty(testName,ts,testNameAll)
...
if(strcmp(testName,'BinaxNOW')) 
    [MLE_RTPCR,MLE_Ag,~,U_Ag,MLE_PPA,U_PPA,Dt,totalpos,truepos,w,t,CCtestRTPCR,CCtest,SymP] = Sensitivity_for_Plotting('BinaxNOW (FDA)',4.4);
    ...
    patch([t flip(t)],100.*[prctile(U_PPA,2.5) flip(prctile(U_PPA,97.5))],CCtest,'LineStyle','none','FaceAlpha',0.2);
    p1=plot(t,100.*MLE_PPA,'-','color',CCtest,'LineWidth',2); 
...
elseif(strcmp(testName,'BinaxNOW (Community)'))
    ...
    S2 = TestSensitivity(t,ts,inf,betaAg,betaRTPCR);
    ...
    S = TestSensitivity(t,ts,inf,betaAg,betaRTPCR);  
    
    
    SR = TestSensitivity(t,ts,inf,[],betaRTPCR);
...
if(strcmp(testName,'BinaxNOW')) 
    ...
    S = TestSensitivityOLD(t,ts,inf,betaAg,0);  
    
    
    SR = TestSensitivityOLD(t,ts,inf,[],0);
    [CCtestRTPCR,~]=ColourTests('RTPCR');
 ...
 elseif(strcmp(testName,'BinaxNOW (Community)')) 
    ...
    S2 = TestSensitivityOLD(t,ts,inf,betaAg,0);
    [CCtest2,SymP2]= ColourTests(testNameAll{fff});
    CCtest2=CCtest2(1,:);    
