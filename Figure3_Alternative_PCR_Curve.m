...
load([num2str(1) '-day_Delay_Testing_Frequency_RTPCR_DeltaVOC_Alternative_PCR.mat']);
R(:,1)=(1-pA).*RTotS+pA.*RTotA;
RRTPCRDelay=R(:,1);

load([num2str(1) '-day_Delay_Testing_Frequency_RTPCR_DeltaVOC_Alternative_PCR_Uncertainty.mat']);
R_UN(:,:,1)=(1-pA).*RTotSv+pA.*RTotAv;


load('Testing_Frequency_LumiraDX (Anterior Nasal Swab)_DeltaVOC_Alternative_PCR.mat')
R(:,2)=(1-pA).*RTotS+pA.*RTotA;
load('Testing_Frequency_LumiraDX (Anterior Nasal Swab)_DeltaVOC_Alternative_PCR_Uncertainty.mat')
R_UN(:,:,2)=(1-pA).*RTotSv+pA.*RTotAv;

load('Testing_Frequency_Sofia (FDA)_DeltaVOC_Alternative_PCR.mat')
R(:,3)=(1-pA).*RTotS+pA.*RTotA;
load('Testing_Frequency_Sofia (FDA)_DeltaVOC_Alternative_PCR_Uncertainty.mat')
R_UN(:,:,3)=(1-pA).*RTotSv+pA.*RTotAv;


load('Testing_Frequency_BinaxNOW (FDA)_DeltaVOC_Alternative_PCR.mat')
R(:,4)=(1-pA).*RTotS+pA.*RTotA;
load('Testing_Frequency_BinaxNOW (FDA)_DeltaVOC_Alternative_PCR_Uncertainty.mat')
R_UN(:,:,4)=(1-pA).*RTotSv+pA.*RTotAv;

load('Testing_Frequency_BD Veritor_DeltaVOC_Alternative_PCR.mat')
R(:,5)=(1-pA).*RTotS+pA.*RTotA;
load('Testing_Frequency_BD Veritor_DeltaVOC_Alternative_PCR_Uncertainty.mat')
R_UN(:,:,5)=(1-pA).*RTotSv+pA.*RTotAv;

load('Testing_Frequency_CareStart (Anterior Nasal Swab - FDA)_DeltaVOC_Alternative_PCR.mat')
R(:,6)=(1-pA).*RTotS+pA.*RTotA;
load('Testing_Frequency_CareStart (Anterior Nasal Swab - FDA)_DeltaVOC_Alternative_PCR_Uncertainty.mat')
...
for zz=0:5
    load([num2str(zz) '-day_Delay_Testing_Frequency_RTPCR_DeltaVOC_Alternative_PCR.mat']);
    R(:,zz+1)=(1-pA).*RTotS+pA.*RTotA;
    
    
    load([num2str(zz) '-day_Delay_Testing_Frequency_RTPCR_DeltaVOC_Alternative_PCR_Uncertainty.mat']);
...
for kk=1:NumTest
    load(['Testing_Frequency_' testName{kk} '_DeltaVOC_Alternative_PCR.mat'],'RTotS','RTotA');
    RAllAgTest(:,kk)=(1-pA).*RTotS+pA.*RTotA;
    
    load(['Testing_Frequency_' testName{kk} '_DeltaVOC_Alternative_PCR_Uncertainty.mat'],'RTotSv','RTotAv')
...
testFreq=[max(Freq(AgRLT1==18)) prctile(MF,2.5) prctile(MF,97.5)];
...
testFreq=[min(Freq(AgRLT1==0)) prctile(MF,2.5) prctile(MF,97.5)];
fprintf('Bounds for maximum time between testing for all RA tests such that RE>1: %2.0f (%2.0f - %2.0f) \n',testFreq) 
...
print(gcf,'Figure3_Alternative','-dpng','-r300');
