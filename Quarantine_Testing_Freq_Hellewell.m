function [QDurE,QDurEE,FreqT,FPProb] = Quarantine_Testing_Freq_Hellewell(testName,pA,DurT)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
Risk=1;
load('TestingonExit_RTPCR_24hrDelay_Hellewell.mat','q','IDSLS','IDSLA');
RRTPCR=Probability_Onward((1-pA).*IDSLS(q==7)+pA.*IDSLA(q==7),Risk);

load(['TestingonExit_' testName '_NoDelay_Hellewell.mat'],'q','IDSLS','IDSLA')
RAg=Probability_Onward((1-pA).*IDSLS+pA.*IDSLA,Risk);

QDurE=min(q(RAg<RRTPCR));

load(['TestingonEntryExit_' testName '_NoDelay_Hellewell.mat'],'q','IDSLS','IDSLA')
RAg=Probability_Onward((1-pA).*IDSLS+pA.*IDSLA,Risk);
...
load(['Testing_Frequency_' testName '_Hellewell.mat'],'RTotA','RTotS')
