...
for ii=1:NumTest
    load(['Testing_Frequency_' testName{ii} '_DeltaVOC_Alternative_PCR.mat'],'RTotS','RTotA');
    R(:,ii+1)=(1-pA).*RTotS+pA.*RTotA;
    
    load(['Testing_Frequency_' testName{ii} '_DeltaVOC_Alternative_PCR_Uncertainty.mat'],'RTotSv','RTotAv')
...
for fr=4:-1:1
    ...
    tfr=1+(4-fr);
    text(-2.699383553973718,1.489425287356321./1.4.*1.9,char(96+tfr),'Fontsize',34,'FontWeight','bold');
    if(rem(fr,2)==1)
        print(gcf,['Alltests_Testing_Frequency=' num2str(fr) '_Alternative.png'],'-dpng','-r600');
    end
end
