...
for TestN=1:NumTests
    ...
    parfor jj=1:14 
        IDSLS(jj)=((1./ts).*integral2(@(u,t)InfectiousnessfromInfectionTesting(t+u,u,[0 q(jj)],testtype,R0S,R0A,0,ts,td,SelfIsolate,betaRTPCR),0,ts,q(jj),inf));
        IDSLA(jj)=((1./td).*integral2(@(u,t)InfectiousnessfromInfectionTesting(t+u,u,[0 q(jj)],testtype,R0S,R0A,1,ts,td,0,betaRTPCR),0,td,q(jj),inf));  
    end

    save(['TestingonEntryExit_' testName{TestN} '_NoDelay_DeltaVOC_Alternative_PCR.mat']);
end
...
for TestN=1:NumTests
    ...
    for jj=1:14 
        parfor ns=1:NSS
            ...
            IDSLS(ns)=((1./ts).*integral2(@(u,t)InfectiousnessfromInfectionTesting(t+u,u,[0 q(jj)],testtype,R0S,R0A,0,ts,td,SelfIsolate,betaRTPCRv(ns,:)),0,ts,q(jj),inf));
            IDSLA(ns)=((1./td).*integral2(@(u,t)InfectiousnessfromInfectionTesting(t+u,u,[0 q(jj)],testtype,R0S,R0A,1,ts,td,0,betaRTPCRv(ns,:)),0,td,q(jj),inf));  
        end
        IDSLSv(:,jj)=IDSLS;
        IDSLAv(:,jj)=IDSLA;
    end
    save(['TestingEntryExit_' testName{TestN} '_NoDelay_DeltaVOC_Alternative_PCR_Uncertainty.mat']);
end
