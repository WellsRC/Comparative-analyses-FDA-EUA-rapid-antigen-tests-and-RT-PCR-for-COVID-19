...
for TestN=1:NumTests
    ...
    save(['TestingonExit_' testName{TestN} '_NoDelay_DeltaVOC_Alternative_PCR.mat']);
end
...
for TestN=1:NumTests
    ...
    for jj=1:14 
        parfor ns=1:NSS
            ...
            IDSLS(ns)=((1./ts).*integral2(@(u,t)InfectiousnessfromInfectionTesting(t+u,u,[q(jj)],testtype,R0S,R0A,0,ts,td,SelfIsolate,betaRTPCRv(ns,:)),0,ts,q(jj),inf));
            IDSLA(ns)=((1./td).*integral2(@(u,t)InfectiousnessfromInfectionTesting(t+u,u,[q(jj)],testtype,R0S,R0A,1,ts,td,0,betaRTPCRv(ns,:)),0,td,q(jj),inf)); 
        end
        IDSLSv(:,jj)=IDSLS;
        IDSLAv(:,jj)=IDSLA;
    end
    save(['TestingonExit_' testName{TestN} '_NoDelay_DeltaVOC_Alternative_PCR_Uncertainty.mat']);
end
