...
for TestN=1:NumTests
    ...
    parfor jj=1:14 
        IDSLS(jj)=((1./ts).*integral2(@(u,t)InfectiousnessfromInfectionTesting(t+u,u,[q(jj)],testtype,R0S,R0A,0,ts,tL,td,SelfIsolate,betaRTPCR),0,ts,q(jj),inf));
        IDSLA(jj)=((1./td).*integral2(@(u,t)InfectiousnessfromInfectionTesting(t+u,u,[q(jj)],testtype,R0S,R0A,1,ts,tL,td,0,betaRTPCR),0,td,q(jj),inf));  
    end

    save(['TestingonExit_' testName{TestN} '_NoDelay_Hellewell.mat']);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% RT-PCR (OLD)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for TestN=1:NumTests    
    ...
    parfor jj=1:14 
        IDSLS(jj)=((1./ts).*integral2(@(u,t)InfectiousnessfromInfectionTestingOLD(t+u,u,[q(jj)],testtype,R0S,R0A,0,ts,tL,td,SelfIsolate),0,ts,q(jj),inf));
        IDSLA(jj)=((1./td).*integral2(@(u,t)InfectiousnessfromInfectionTestingOLD(t+u,u,[q(jj)],testtype,R0S,R0A,1,ts,tL,td,0),0,td,q(jj),inf));  
    end

    save(['TestingonExit_' testName{TestN} '_NoDelay_NatComm.mat']);
end
clear;
